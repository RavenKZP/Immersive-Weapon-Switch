...
namespace ImGui
{
	Texture::Texture(const std::wstring_view a_path) :
		path(a_path)
	{}

	Texture::Texture(const std::wstring_view a_folder, const std::wstring_view a_textureName)
	{
		...
		if (SUCCEEDED(hr)) {
			if (auto renderer = RE::BSGraphics::Renderer::GetSingleton()) {
				if (a_resizeToScreenRes) {
                    ...
                    logger::trace("{} {}", screenSize.width, screenSize.height);
