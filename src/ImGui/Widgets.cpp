...
namespace ImGui
{
	...
	bool ImGui::CenteredTextWithArrows(const char* label, const std::string_view centerText)
	{
		ImGuiWindow* window = GetCurrentWindow();
		if (window->SkipItems)
			return false;
	...
	bool DragScalarEx(const char* label, const ImGuiDataType data_type, void* p_data, const float v_speed, const void* p_min, const void* p_max, const char* format, const ImGuiSliderFlags flags)
	{
		ImGuiWindow* window = GetCurrentWindow();
		if (window->SkipItems)
			return false;
	...
	bool DragFloatEx(const char* label, float* v, const float v_speed, const float v_min, const float v_max, const char* format, const ImGuiSliderFlags flags)
	{
		return DragScalarEx(label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max, format, flags);
	}

	bool DragIntEx(const char* label, int* v, const float v_speed, const int v_min, const int v_max, const char* format, const ImGuiSliderFlags flags)
	{
		return DragScalarEx(label, ImGuiDataType_S32, v, v_speed, &v_min, &v_max, format, flags);
	}

	bool ThinSliderScalar(const char* label, const ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, const ImGuiSliderFlags flags, const float sliderThickness)
	{
		ImGuiWindow* window = GetCurrentWindow();
		if (window->SkipItems)
			return false;
	...
	bool ThinSliderFloat(const char* label, float* v, const float v_min, const float v_max, const char* format, const ImGuiSliderFlags flags)
	{
		return ThinSliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags, 0.5f);
	}

	bool ThinSliderInt(const char* label, int* v, const int v_min, const int v_max, const char* format, const ImGuiSliderFlags flags)
	{
		return ThinSliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags, 0.5f);
	}

	bool BeginTabItemEx(const char* label, ImGuiID* active_tab, bool* p_open = nullptr, const ImGuiTabItemFlags flags = 0)
	{
