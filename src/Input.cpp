...
namespace Input
{
	...
	ImGuiKey Manager::ToImGuiKey(const KEY a_key)
	{
		switch (a_key) {
		case KEY::kTab:
			return ImGuiKey_Tab;
	...
	ImGuiKey Manager::ToImGuiKey(const GAMEPAD_DIRECTX a_key)
	{
		switch (a_key) {
	...
	ImGuiKey Manager::ToImGuiKey(const GAMEPAD_ORBIS a_key)
	{
		switch (a_key) {
